...
class AITestScenarioGenerator:
    ...
    def generate_scenarios(self, context_runs=None, num_scenarios=5):
        ...
        run_summary = json.dumps(context_runs[:3], indent=2)  # Use top 3 runs for context
...
The fleet operates under ROS2 Jazzy in Gazebo Harmonic. Robot topics and frames use the /robot_001/ namespace, and scenarios should reflect a multi-robot fleet deployment rather than Isaac Sim / OmniGraph workflows.

Recent test run data:
{run_summary}

Based on this telemetry, generate {num_scenarios} edge case test scenarios for a multi-robot fleet operating in Gazebo Harmonic with ROS2 Jazzy.
...
Generate {num_scenarios} creative, realistic scenarios:"""
    ...
    def store_scenarios(self, scenarios, run_id, usage):
        ...
        for scenario in scenarios:
            cursor.execute("""
                ...
                (run_id, scenario_name, suggested_description, ai_model, prompt_tokens, response_tokens)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                run_id,
    ...
    def run_generation_pipeline(self, num_scenarios=5):
        ...
        if scenarios:
            ...
            print(f"Scenario generation complete. Used {usage.input_tokens} input tokens, {usage.output_tokens} output tokens.")
            return scenarios
        else:
